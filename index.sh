...
wait_for_port_then_succeed_or_die_with_pid() {
    local pid=$1
    local port=$2
...
if ! check_process $SERVER_PID_FILE || ! check_process $UI_PID_FILE; then
    ...
    SERVER_PID=$!
    wait_for_port_then_succeed_or_die_with_pid $SERVER_PID $SERVER_PORT
    ...
    wait_for_port_then_succeed_or_die_with_pid $UI_PID $UI_PORT
